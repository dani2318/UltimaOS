...
public:
    ...
    {
        gScreenwriter->print("Disk: Initializing IDE controller...\n", true);
        ...
        if (!WaitReady(5000))
        {
            gScreenwriter->print("Disk: Drive not ready after reset\n", true);
            return false;
        }
        ...
        if (status == 0xFF)
        {
            gScreenwriter->print("Disk: No drive detected\n", true);
            return false;
        }

        gScreenwriter->print("Disk: IDE controller initialized\n", true);
        return true;
    }
    ...
    static bool ReadSectors(uint64_t lba, uint8_t sector_count, void *buffer)
    {
        ...
        if (!WaitReady(1000))
        {
            gScreenwriter->print("Disk: Not ready\n", true);
            asm volatile("sti");
            return false;
        }
        ...
        for (int sector = 0; sector < sector_count; sector++)
        {
            if (!WaitReady(1000))
            {
                gScreenwriter->print("Disk: Read timeout\n", true);
                asm volatile("sti");
                return false;
            }
...
public:
    ...
    {
        gScreenwriter->print("GPT: Reading partition table...\n", true);
        ...
        {
            gScreenwriter->print("GPT: Failed to read header\n", true);
            return {0, 0, false};
        }
        ...
        {
            gScreenwriter->print("GPT: Invalid signature\n", true);
            return {0, 0, false};
        }

        gScreenwriter->print("GPT: Valid header found\n", true);
        ...
        {
            gScreenwriter->print("GPT: Invalid entry size\n", true);
            return {0, 0, false};
        }
        ...
        {
            gScreenwriter->print("GPT: Failed to read entries\n", true);
            return {0, 0, false};
        }
        ...
        {
            gScreenwriter->print("GPT: Partition not found at index\n", true);
            return {0, 0, false};
        }

        gScreenwriter->print("GPT: Partition found at LBA: ", true);
        char buf[32];
        gScreenwriter->print(itoa(entry->starting_lba, buf, 10), true);
        gScreenwriter->print("\n", true);
