...
void* ACPIDriver::MapTable(uint64_t phys_addr, PageTableManager &manager)
{
    ...
    manager.mapPage(aligned_phys, aligned_phys, true, false, true); // writable=false, user=false, executable=true
    ACPIHeader* header = (ACPIHeader*)PHYS_TO_VIRT(phys_addr);

    uint32_t total_len = header->Length;
    uint64_t pages = (total_len + 4095) / 4096; // ceil division

    for (uint64_t i = 0; i < pages; ++i)
    {
        uint64_t p = aligned_phys + i * 4096;
        manager.mapPage(p, p, true, false, true); // same flags
    }
